...
|------------------------|---------------------------------------------------------------------|---------------|
| `CERTBOT_DOMAINS`      | Список доменов в кавычках, разделенных запятыми, для которых необходимо получить сертификат. В случае если необходим `wildcard` сертификат, первым указывается сам домен. Пример: "domain.tld,*.domain.tld"  | - |
